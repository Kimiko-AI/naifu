...
from models.lumina import NextDiT_2B_GQA_patch2_Adaln_Refiner as Lumina_2b
...
class Lumina2Model(pl.LightningModule):
    ...
    def build_models(self):
        ...
        ckcp = torch.load("checkpoints/lumina_gradient/checkpoint-e0_s12000.ckpt", weights_only = False)
        ...
        if self.config.trainer.get("auto_resume", False) and self.config.trainer.resume is None:
            try:
                ...
                if len(existing_checkpoints) > 0:
                    ...
                    self.config.trainer.resume = os.path.join(self.config.trainer.checkpoint_dir,
        ...
        if self.config.model.get("resume", None) is not None:
            ...
            if hasattr(self, "model_ema") and os.path.exists(
                    os.path.join(self.config.model.resume, "consolidated_ema.00-of-01.pth")):
    ...
    def encode_images(self, images):
        ...
        # Split back into list (same structure as before)
        latents_list = [lat.float() for lat in latents]
